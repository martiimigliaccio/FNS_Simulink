...
rng(42)  % riproducibilità anche per inizializzazione pesi rete
...
disp(h_net.Learnables)   % diagnostica per vedere se la tabella è vuota o no
...
% De-normalizzazione: y = y_norm .* std_out + mu_out
output_data_phys = output_data(idx_test,:) .* std_out + mu_out;
y_pred_phys      = y_pred.OutputData      .* std_out + mu_out;

figure('Name','Neural State Space — Validazione finale')
titles = {'Fz_{sl} [N]', 'Fx_{sl} [N]', 'My_{sl} [N·m]'};
for k = 1:3
    subplot(3,1,k)
    plot(t_test(idx_plt), output_data_phys(idx_plt,k), ...
        'b',  'LineWidth', 1.5, 'DisplayName', 'Pendolo riferimento')
    hold on
    plot(t_test(idx_plt), y_pred_phys(idx_plt,k), ...
